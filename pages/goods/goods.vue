<template>
	<view class="u-wrap">
		<template v-if="Object.keys(navData).length">
			<view class="u-search-box"><u-search placeholder="搜索商品" v-model="keyword" @search="searchGoods" @custom="searchGoods"></u-search></view>
			<view class="u-menu-wrap">
				...
				<block>
					<scroll-view scroll-y class="right-box" :lower-threshold="30" @scrolltolower="toLower">
						<view class="u-text-center" v-if="isShowLoading"><u-loading color="#2979ff"></u-loading></view>
						<template v-else>
							...
							<view v-else class="page-view">
								<view class="class-item">
									<view class="item-container">
										<navigator
											class="thumb-box u-p-10"
											v-for="(goods, index) in goodsList"
											:key="index"
											:url="`/pages/goods/goods-detail/goods-detail?id=${goods.id}`"
										>
											<image class="item-menu-image" :src="goods.cover_url" mode=""></image>
											<view class="item-menu-name u-line-1">{{ goods.title }}</view>
										</navigator>
									</view>
									<u-loadmore @loadmore="toLower" :status="loadStatus" icon-color="#2979ff" color="#2979ff" />
								</view>
							</view>
						</template>
					</scroll-view>
		...
		<template v-else>
			<view class="loading">
				...
				<view><text class="small">拼命加载中~</text></view>
			</view>
		</template>
	</view>
</template>
